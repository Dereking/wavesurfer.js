/**
 * @typedef {Object} TimelinelrcPluginParams
 * @desc Extends the `WavesurferParams` wavesurfer was initialised with
 * @property {!string|HTMLElement} container CSS selector or HTML element where
 * the timelinelrc should be drawn. This is the only required parameter.
 ...
 * getDuration() for setting length of timelinelrc
 ...
 * @property {?number} offset Offset for the timelinelrc start in seconds. May also be
 * negative.
 * @property {?boolean} deferInit Set to true to manually call
 * `initPlugin('timelinelrc')`
 */


/**
 * @typedef {Object} LrclineParams
 * @desc The parameters used to describe a Lrcline.
 * @example wavesurfer.addMarker(regionParams);
 * @property {number} time The time to set the Lrcline at
 * @property {?label} string An optional Lrcline label
 * @property {?color} string Background color for Lrcline
 * @property {?position} string "top" or "bottom", defaults to "bottom"
 * @property {?lrclineElement} element An HTML element to display instead of the default Lrcline image
 * @property {?draggable} boolean Set Lrcline as draggable, defaults to false
 */

/**
 * Adds a timelinelrc to the waveform.
 *
 * @implements {PluginClass}
 * @extends {Observer}
 * @example
 * // es6
 * import TimelinelrcPlugin from 'wavesurfer.timelinelrc.js';
 *
 * // commonjs
 * var TimelinelrcPlugin = require('wavesurfer.timelinelrc.js');
 *
 * // if you are using <script> tags
 * var TimelinelrcPlugin = window.WaveSurfer.timelinelrc;
 *
...
export default class TimelinelrcPlugin {
    /**
     * @typedef {Object} TimelinelrcPluginParams
     * @property {?LrclineParams[]} lrclines Initial set of lrclines
     * @fires TimelinelrcPlugin#lrcline-click
     ...
     *
     * @param  {TimelinelrcPluginParams} params parameters use to initialise the plugin
    ...
    static create(params) {
        return {
            ...
            staticProps: {
                addLrcline(options) {
                    if (!this.initialisedPluginList.lrclines) {
                        this.initPlugin('timelinelrc');
                    }
                    return this.lrclines.add(options);
                },
                clearLrclines() {
                    this.lrclines && this.lrclines.clear();
                }
            },
            instance: TimelinelrcPlugin
        };
    }
    ...
    /**
     * Creates an instance of TimelinelrcPlugin.
     *
     * You probably want to use TimelinelrcPlugin.create()
     *
     * @param {TimelinelrcPluginParams} params Plugin parameters
    ...
    constructor(params, ws) {
        ...
        if (!this.container) {
            throw new Error('No container for wavesurfer timelinelrc');
        }
     ...
     * Add a lrc line
     *
     * @param {LrclineParams} params Lrc line definition
     * @return {object} The created lrc line
     */
     add(params) {
        let lrcline = {
            time: params.time,
        ...
        //lrcline.el = this._createMarkerElement(lrcline, params.lrclineElement);

        //this.wrapper.appendChild(lrcline.el);
        this.lrclines.push(lrcline);
    ...
    createWrapper() {
        ...
        this.wrapper = this.container.appendChild(
            document.createElement('timelinelrc')
        );
    ...
    /**
     * Render the timelinelrc (also updates the already rendered timelinelrc)
     *
     */
    render() {
        if (!this.wrapper) {
    ...
    /**
     * Make sure the correct of timelinelrc canvas elements exist and are cached in
     * this.canvases
     *
     */
    updateCanvases() {
    ...
    /**
     * Update the dimensions and positioning style for all the timelinelrc canvases
     *
     */
    ...
    /**
     * Render the timelinelrc labels and notches
     *
     */
    renderCanvases() {
        const duration =
        ...
        this.setFonts(`${fontSize}px ${this.params.fontFamily}`);
        this.setFillStyles(this.params.primaryFontColor);
        for (i = 0; i < this.lrclines.length; i++) {
            var l = this.lrclines[i]
            this.fillText(l.label, 
                pixelsPerSecond * l.time, 30 );
        }
    }
